...
Pattern_Wall_Thickness = 0.5;
Pattern_Size = 15;
xstep = cos(30) * (Pattern_Wall_Thickness + sqrt(3) * Pattern_Size / 2);
ystep = sin(30) * (Pattern_Wall_Thickness + sqrt(3) * Pattern_Size / 2);
xmove = [ 2 * xstep, 0];
ymove = [ xstep, ystep ];
...
module sideWall(side = "left"){
    xoffset = side == "left" ? -Outer_Width/2 : 
        side == "right" ? Outer_Width/2 : 0;
    zoffset = side == "left" || side == "right"? Outer_Height/2 : 0;
    echo(str("xoffset: ", xoffset, " zoffset: ", zoffset));
    translate([xoffset, 0, 0]) rotate([0, 0, 0]) {
        patterned_wall(Wall_Thickness, Inner_Width, Inner_Depth) {
            difference(){
                square([Outer_Width, Outer_Depth], center=true);
            }
            spray_pattern([[-20,-20], [Inner_Width, Inner_Depth]], [ xmove, ymove])
                cylinder(d = Pattern_Size, h = Wall_Thickness, center = true, $fn = 6);
        }
    }
