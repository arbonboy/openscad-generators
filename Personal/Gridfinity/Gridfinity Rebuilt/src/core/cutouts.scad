 ...
 */
 function cgs(size=[1, 1], height=0, divider_width=0) =
    assert(is_valid_2d(size) && is_positive(size))
    assert(is_num(height))
    assert(is_num(divider_width))   
    let(div_width = divider_width == 0 ? d_div : divider_width)
    ...
    let(divider_mm = [div_width/2, div_width/2, 0])
