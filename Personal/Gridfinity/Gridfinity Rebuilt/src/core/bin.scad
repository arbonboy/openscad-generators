...
module bin_subdivide(bin, subdivisions) {
    ...
    echo(str("Subdividing bin into ", num_elements, " elements."));
    infill_size_mm = bin_get_infill_size_mm(bin);
    // infill_size_mm =  [123.6, 81.6, 33.8];
    echo(str("Infill size (mm): ", infill_size_mm));


    if(infill_size_mm.z > 0) {
        grid = grid_from_total(num_elements, infill_size_mm, true);
        // grid = ["grid_struct", [2, 1, 1], [61.8, 39.6, 33.8], true, [0, 0, 0, 0, 0, 0]];
        echo(str("grid: ", grid));
