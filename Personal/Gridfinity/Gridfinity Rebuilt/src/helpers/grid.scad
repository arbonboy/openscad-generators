...
module grid_translate(grid, index, center=false) {
    ...
    // echo(str("grid_translate -> index: ", index));
    // echo(str("grid_translate -> element: ", element));
    ...
    // echo(str("grid_translate -> partial_index: ", partial_index));
    // echo(str("grid_translate -> element_dimensions: ", element_dimensions));
    offset = [for(i=[0:len(index)-1])
        partial_index[i] * element_dimensions[i]
    ];

    // echo(str("grid_translate -> offset: ", offset));
    // echo(str("grid_translate -> element position: ", grid_element_get_position(element)));
    // offset = [10, 10];
    // pos = grid_element_get_position(element) + offset;
    // pos = [pos[0], pos[1]];
    // echo(str("pos: ", pos));
...
module grid_foreach(grid, center_elements=false) {
    assert(is_grid(grid), "Not a grid.");
    num_elements = grid[1];
    // echo(str("grid_foreach -> num_elements: ", num_elements));

    count = grid_get_element_count(grid);
    // echo(str("grid_foreach -> count: ", count));
    rank = len(num_elements);
    // echo(str("grid_foreach -> rank: ", rank));
