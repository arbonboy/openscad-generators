...
depth = 0;  //.1
// Width of the divider between compartments. Leave zero for default. Units: mm
divider_width = 2; //.1
...
/* [Hidden Parameters] */
gridx = half_grid ? xCells * 2 : xCells;
gridy = half_grid ? yCells * 2 : yCells;
...
bin_render(bin1) {
    bin_subdivide(bin1, [divx, divy]) {
        depth_real = cgs(height=depth, divider_width=divider_width).z;
        if (cut_cylinders) {
            cut_chamfered_cylinder(cd/2, depth_real, c_chamfer);
        } else {
            cut_compartment_auto(
                cgs(height=depth, divider_width=divider_width),
                style_tab,
                place_tab != 0,
                scoop
            );
