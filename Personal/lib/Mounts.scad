...
module mnts_postClip(wallThickness=2, generateFastener=true, generateSideClip=true, width=20, baseThickness=2, height=30, connectorTolerance=0.1){
...
module mnts_slideLockFastener(mountingWallThickness=4, fastenerWallThickness=2, clipWallThickness=2, generateFastener=true, generateSideClip=true, generateHole=false, width=20, height=30, connectorClipTolerance=0.15, wallOffset=0){
    ...
    if(generateSideClip){
        translate([0, -fullFastenerThickness*2, fastenerWallThickness]){
            difference(){
                ...
                translate([0, -(fullClipThickness/2), 0]) {
                    rotate([90, 90, 0]){
                        threadedRodForHole(length = fastenerWallThickness*2, center = true, tolerance = 0.2, hole_radius=TB_SCREW_Threaded_Rod_Diameter/2);
                    }
                }
            }
...
module mnts_screwFastener(width=20, height=30, mountingWallThickness=4, fastenerWallThickness=2,fastenerArmLength=20, screwHoleDiameter=10, screwHoleDepth=1, screwHeadDiameter=10, screwHeadDepth=2, generateFastener=true, generateHole=false){
    ...
    if(generateFastener){
        // translate([0, fullFastenerLength+mountingWallThickness/2, 0]){
        //     cube([width, mountingWallThickness, height], center=true);
        // }
        difference(){
            union(){
            ...
            translate([width/2-screwHeadDepth/2+fastenerWallThickness, fastenerArmLength/2, 0]){
                rotate([90, 0, -90]){
        ...
        difference(){
            ...
            translate([-width/2-fastenerWallThickness+screwHeadDepth/2, fastenerArmLength/2, 0]){
                rotate([-90, 0, -90]){
                    cylinder(d=screwHeadDiameter, h=screwHeadDepth, center=true);
                    translate([0, 0, screwHeadDepth/2+screwHoleDepth/2]){
                        cylinder(d=screwHoleDiameter, h=screwHoleDepth, center=true);
...
module mnts_tBScrewHole(wallThickness=2, width=2, baseThickness=2, height=10, wallHeight=-1, zRotation=0, holeLength=-1, generateComponent=true, generateHole=false){
    fullWallThickness = wallThickness + baseThickness;
    officialHoleLength = holeLength==-1 ? fullWallThickness*2 : holeLength;
    officialWallHeight = wallHeight == -1 ? height : wallHeight;

    echo(str("holeLength: ", holeLength));
    if(generateComponent){
        rotate([0,0,zRotation]){
            difference(){
                translate([0, -officialWallHeight/2+height/2, 0]){
                    cube([width+baseThickness, height, fullWallThickness], center=true);
                }
                translate([0, 0, 0]){
    ...
    if(generateHole){
        rotate([0,0,zRotation]){
            threadedRodForHole(length = officialHoleLength*2, center = true, tolerance = 0.2, hole_radius=TB_SCREW_Threaded_Rod_Diameter/2);
        }
    }
...
module mnts_rectHolder(mountWidth=40, mountHeight=40, mountingWallThickness=4, holderWidth=50, holderHeight=20, holderWallThickness=3, holderDepth=20){
    fullRectHolderLength = mountingWallThickness + holderDepth;
    translate([0, 0, mountHeight/2]){
        //Mount
        cube([mountWidth, mountingWallThickness, mountHeight], center=true);
        //Holder
        translate([0, -fullRectHolderLength/2,-(mountHeight-holderHeight)/2 ]){
            difference(){
...
module mnts_circularHolder(mountWidth=40, mountHeight=40, mountingWallThickness=8, holderDiameter=80, holderHeight=40, holderWallThickness=4){
    fullCircularHolderLength = mountingWallThickness + holderDiameter;
    translate([0, 0, mountHeight/2]){
        //Mount
        cube([mountWidth, mountingWallThickness, mountHeight], center=true);
        //Holder
        translate([0, -fullCircularHolderLength/2+1,-(mountHeight-holderHeight)/2 ]){
            difference(){
...
module mnts_TBScrewFastener(width=20, height=30, mountingWallThickness=4, fastenerWallThickness=2,fastenerArmLength=20, threadLengthPadding=6, generateFastener=true, generateTBScrew=true, generateHole=false){
    fullFastenerLength = mountingWallThickness+fastenerArmLength;
    threadLength = width + fastenerWallThickness*2 + threadLengthPadding;
    headHeight=8;
    ...
    if(generateFastener){
        difference(){
            ...
            translate([0, fullFastenerLength/2, 0]){
                rotate([90,0,90]){
                    threadedRodForHole(length = threadLength, center = true, tolerance = 0.2, hole_radius=TB_SCREW_Threaded_Rod_Diameter/2);
                    translate([0, 0, -threadLength/2]){
                        cylinder(h=threadLength, d=TB_SCREW_Threaded_Rod_Diameter+1, center=true);
                    }
                }
            }
        }
    ...
    if(generateTBScrew){
        translate([0, 0, (threadLength/2-headHeight*2)]){
            rotate([0, 0, 0]){
                screw(rodHeight = threadLength, headHeight = headHeight, headRadius = 10.5, headStyle = "hex", center = true);
            }
        }
    }
