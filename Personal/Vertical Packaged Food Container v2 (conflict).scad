...
module main(){
    difference() {
        union(){
            translate([0, -100, 0]) rotate([0, 0, 0]) {
                patterned_wall(thickness, height, reinforcement_height) {
                    square([Outer_Width, Outer_Depth], center=true);
                    spray_pattern([[-20,-20], [Inner_Width, Inner_Depth]], [ xmove, ymove])
                        cylinder(d = alveolus_size, h = Wall_Thickness, center = true, $fn = 6);
                }
            }
...
module patterned_wall(thickness, height, reinforcement_height) {
