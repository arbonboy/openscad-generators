...
module tb_ntb_board(rows=2, cols=2, thickness=DEFAULT_TB_NTB_Thickness, cell_size=TB_NTB_Cell_Size, hole_radius=TB_NTB_Hole_Radius, roundedCorners = false, cornerRadius = 0.5){
...
module tb_ntb_countersinkPeg(rows = 1, cols = 1, headRadius=11, headHeight=8, stemRadius=TB_NTB_Hole_Radius, stemHeight=20){
    for (row = [0:rows-1]){
        for (col = [0:cols-1]){
            translate([col*TB_NTB_Cell_Size, row*TB_NTB_Cell_Size, 0]){
